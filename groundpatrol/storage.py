...
def _is_writable_directory(path: Path) -> bool:
    ...
    AgentCore CodeZip exposes packaged application paths as non-writable runtime
    content. A directory may therefore exist while still rejecting writes, so merely
    calling mkdir(exist_ok=True) is not enough.
    """
    try:
